...
@onready var eased_healrh_bar: TextureProgressBar = $V/HealthBar/EasedHealrhBar
...
func update_health(skip_anim := false) -> void:
	...
	if skip_anim:
		eased_healrh_bar.value = percentage
	else:
		create_tween().tween_property(eased_healrh_bar, "value", percentage, 0.3)
